class FavoritesController < ApplicationController
  def create
    ...
    respond_to do |format|
      ...
      format.json { render json: {status: 'favorited', message: message } }
    end
  end
  ...
  def destroy
    ...
    if favorite
      favorite.destroy
      message = "#{favorite.menu.title}をお気に入り解除しました！"
    else
    ...
    respond_to do |format|
      format.html { redirect_to menus_path, notice: message }
      format.json { render json: {status: 'unfavorited', message: message } }
    end
  end
end 
